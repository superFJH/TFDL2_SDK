    ...
    if args.check:
        if convertor.verification(targetModel=args.testonnxpath):
            ...
            if args.quantimgDir is not None:
                ...
                if args.detectOnly:
                    convertor.quantContext(calibration_list=imglist,decoderflags=DECODER_FLAGS(args.cvtype),MergeConcate=False,avoidtensors=("/model.23/dfl/conv/Conv",),stopquanttensors=("/model.23/Reshape","/model.23/Reshape_1","/model.23/Reshape_2"))
                else:
                    convertor.quantContext(calibration_list=imglist,decoderflags=DECODER_FLAGS(args.cvtype),MergeConcate=False,avoidtensors=("/model.23/dfl/conv/Conv",),stopquanttensors=("/model.23/Reshape","/model.23/Reshape_1","/model.23/Reshape_2","/model.23/Reshape_3","/model.23/Reshape_4","/model.23/Reshape_5"))
            else:
                print("没有量化图片集，使用随机数量化")
                if args.detectOnly:
                    convertor.quantContext(calibration_list=None,decoderflags=DECODER_FLAGS(args.cvtype),MergeConcate=False,avoidtensors=("/model.23/dfl/conv/Conv",),stopquanttensors=("/model.23/Reshape","/model.23/Reshape_1","/model.23/Reshape_2"))
                else:
                    convertor.quantContext(calibration_list=None,decoderflags=DECODER_FLAGS(args.cvtype),MergeConcate=False,avoidtensors=("/model.23/dfl/conv/Conv",),stopquanttensors=("/model.23/Reshape","/model.23/Reshape_1","/model.23/Reshape_2","/model.23/Reshape_3","/model.23/Reshape_4","/model.23/Reshape_5"))
            if args.fp16:
                convertor.toFp16()
