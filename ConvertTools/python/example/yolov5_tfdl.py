    ...
    if args.check:
        if convertor.verification(targetModel=args.testonnxpath):
            ...
            if args.quantimgDir is not None:
                ...
                convertor.quantContext(calibration_list=imglist,decoderflags=DECODER_FLAGS(args.cvtype),MergeConcate=False,avoidtensors=("/model.24/dfl/conv/Conv",),stopquanttensors=("/model.24/Reshape","/model.24/Reshape_1","/model.24/Reshape_2"))
            else:
