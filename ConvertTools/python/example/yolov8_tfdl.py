    ...
    if args.check:
        if convertor.verification(targetModel=args.testonnxpath):
            ...
            if args.quantimgDir is not None:
                ...
                if args.detectOnly:
                    convertor.quantContext(calibration_list=imglist,decoderflags=DECODER_FLAGS(args.cvtype),MergeConcate=False,avoidtensors=("/model.22/dfl/conv/Conv",),stopquanttensors=("/model.22/Reshape","/model.22/Reshape_1","/model.22/Reshape_2"))
                else:
                ...
                if args.detectOnly:
                    convertor.quantContext(calibration_list=None,decoderflags=DECODER_FLAGS(args.cvtype),MergeConcate=False,avoidtensors=("/model.22/dfl/conv/Conv",),stopquanttensors=("/model.22/Reshape","/model.22/Reshape_1","/model.22/Reshape_2"))
                else:
