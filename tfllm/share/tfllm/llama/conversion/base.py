...
class MmprojModel(ModelBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tensor_map = gguf.get_tensor_name_map(gguf.MODEL_ARCH.MMPROJ, max(5, self.block_count or 0))
...
def get_model_architecture(hparams: dict[str, Any], model_type: ModelType) -> str:
    ...
    arch = None
    # TFLLM: current Transformers serializes VL wrapper names in text_config.
    nested = text_config.get("architectures") or []
    if not hparams.get("architectures") and nested and nested[0] in ("Qwen2VLForConditionalGeneration", "Qwen2_5_VLForConditionalGeneration"):
        arch = nested[0]
    ...
    if model_type == ModelType.TEXT and arch == "LocateAnythingForConditionalGeneration":
        return "TFLLMLocateText"
    # TFLLM LocateAnything slow/AR mode uses its Qwen2 text sub-config.
    if model_type == ModelType.MMPROJ and arch == "LocateAnythingForConditionalGeneration":
        return "TFLLMLocateVision"

    # TFLLM dense HF wrappers; preserve the original checkpoint on disk.
    if model_type == ModelType.MMPROJ and arch in ("LlavaForConditionalGeneration", "LlavaNextForConditionalGeneration") and vision_config.get("model_type") in ("clip_vision_model", "siglip_vision_model"):
        return "TFLLMDenseVision"
    if model_type == ModelType.TEXT and arch in ("LlavaForConditionalGeneration", "LlavaNextForConditionalGeneration", "Mistral3ForConditionalGeneration", "InternVLForConditionalGeneration", "InternVLChatModel") and not text_config.get("architectures"):
        dense = {"llama": "LlamaForCausalLM", "mistral": "LlamaForCausalLM", "qwen2": "Qwen2ForCausalLM", "qwen3": "Qwen3ForCausalLM"}
        if text_config.get("model_type") in dense:
            return dense[text_config["model_type"]]
    if model_type == ModelType.MMPROJ and arch == "InternVLForConditionalGeneration":
        return "TFLLMInternVision"
