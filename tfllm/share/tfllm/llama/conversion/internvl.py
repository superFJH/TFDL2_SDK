...
class InternVisionModel(MmprojModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # TFLLM: original InternVL checkpoints embed preprocessing in Python,
        # not preprocessor_config.json. These are its ImageNet defaults.
        ...
        if self.global_config.get("ps_version", "v2") != "v2":
            raise ValueError("InternVL ps_version=v1 transposed merger is not supported")
        if self.global_config.get("select_layer", -1) != -1:
            raise ValueError("InternVL currently requires last-layer vision features")
