...
namespace TFDLBOX {
    ...
    bool SortBBoxAscend(const NormalizedBBox &bbox1, const NormalizedBBox &bbox2) {
        return bbox1.score_ < bbox2.score_;
    }
...
// Explicit initialization.
    ...
    void ClipBBox(const NormalizedBBox &bbox, NormalizedBBox *clip_bbox) {
        ...
        clip_bbox->size_ = BBoxSize(*clip_bbox);
    ...
    void ScaleBBox(const NormalizedBBox &bbox, const int height, const int width,
        ...
        scale_bbox->size_ = (BBoxSize(*scale_bbox, normalized));
    ...
    void EncodeBBox(
        ...
        if (code_type == PriorBoxParameter_CodeType_CORNER) {
            if (encode_variance_in_target) {
                ...
                TFCHECK_EQ(prior_variance.size(), 4);
                for (int i = 0; i < prior_variance.size(); ++i) {
                    TFCHECK_GT(prior_variance[i], 0);
                }
            ...
            TFCHECK_GT(prior_width, 0);
            float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
            TFCHECK_GT(prior_height, 0);
            ...
            TFCHECK_GT(bbox_width, 0);
            float bbox_height = bbox.ymax() - bbox.ymin();
            TFCHECK_GT(bbox_height, 0);
    ...
    void DecodeBBox(
        ...
        if (code_type == PriorBoxParameter_CodeType_CORNER) {
            ...
            float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
            TFCHECK_GT(prior_height, 0);
            float prior_center_x = (prior_bbox.xmin() + prior_bbox.xmax()) * 0.5;
            float prior_center_y = (prior_bbox.ymin() + prior_bbox.ymax()) * 0.5;
            ...
            decode_bbox->xmin_ = (decode_bbox_center_x - decode_bbox_width * 0.5);
            decode_bbox->ymin_ = (decode_bbox_center_y - decode_bbox_height * 0.5);
            decode_bbox->xmax_ = (decode_bbox_center_x + decode_bbox_width * 0.5);
            decode_bbox->ymax_ = (decode_bbox_center_y + decode_bbox_height * 0.5);
        } else if (code_type == PriorBoxParameter_CodeType_CORNER_SIZE) {
            float prior_width = prior_bbox.xmax() - prior_bbox.xmin();
            TFCHECK_GT(prior_width, 0);
            float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
    ...
    void DecodeBBoxes(
        ...
        TFCHECK_EQ(prior_bboxes.size(), prior_variances.size());
        TFCHECK_EQ(prior_bboxes.size(), bboxes.size());
        int num_bboxes = prior_bboxes.size();
        if (num_bboxes >= 1) {
            TFCHECK_EQ(prior_variances[0].size(), 4);
        }
        decode_bboxes->clear();
    ...
    void DecodeBBoxesAll(const vector<LabelBBox> &all_loc_preds,
        ...
        TFCHECK_EQ(all_loc_preds.size(), num);
    ...
    void GetGroundTruth(const Dtype *gt_data, const int num_gt,
        ...
        for (int i = 0; i < num_gt; ++i) {
            ...
            TFCHECK_NE(background_label_id, label);
...
// Explicit initialization.
    ...
    void GetConfidenceScores(const Dtype *conf_data, const int num,
                             ...
                             vector<map<int, vector<float>>> *conf_preds, int filter) {
        conf_preds->clear();
        conf_preds->resize(num);
        for (int i = 0; i < num; ++i) {
            map<int, vector<float> > &label_scores = (*conf_preds)[i];
            for (int c = 0; c < num_classes; ++c) {
                label_scores[c].resize(num_preds_per_class);
            }

            if (filter != -1) {
                for (int p = 0; p < num_preds_per_class; ++p) {
                    label_scores[filter][p] = (conf_data[p * num_classes + filter]);
                }
            } else {
                for (int p = 0; p < num_preds_per_class; ++p) {
                    int start_idx = p * num_classes;
                    for (int c = 0; c < num_classes; ++c) {
                        label_scores[c][p] = (conf_data[start_idx + c]);
                    }
                }
            }
    ...
    void GetConfidenceScores(const uint8_t *conf_data, const float* conf_answer, const int num,
                             const int num_preds_per_class, const int num_classes,
                             vector<map<int, vector<float> > > *conf_preds, int filter) {
        conf_preds->clear();
        conf_preds->resize(num);
        for (int i = 0; i < num; ++i) {
            ...
            if (filter != -1) {
                for (int p = 0; p < num_preds_per_class; ++p) {
                    label_scores[filter][p] = conf_answer[(conf_data[p * num_classes + filter])];
                }
            } else {
                for (int p = 0; p < num_preds_per_class; ++p) {
                    int start_idx = p * num_classes;
                    for (int c = 0; c < num_classes; ++c) {
                        label_scores[c][p] = conf_answer[(conf_data[start_idx + c])];
                    }
                }
            }
...
// Explicit initialization.
    template void GetConfidenceScores(const float *conf_data, const int num,
                                      const int num_preds_per_class, const int num_classes,
                                      vector<map<int, vector<float>>

                                      > *conf_preds, int filter);

    template void GetConfidenceScores(const double *conf_data, const int num,
                                      const int num_preds_per_class, const int num_classes,
                                      vector<map<int, vector<float>>

                                      > *conf_pred, int filters);

    template<typename Dtype>
    void GetConfidenceScores(const Dtype *conf_data, const int num,
                             const int num_preds_per_class, const int num_classes,
                             const bool class_major, vector<map<int, vector<float>>
            ...
            if (class_major) {
                ...
                for (
                    ...
                    for (
                            ...
                            ++c) {
                        label_scores[c].
                                push_back(conf_data[start_idx + c]);
                    }
                }
                conf_data +=
                        num_preds_per_class * num_classes;
            }
        }
    }
...
// Explicit initialization.
    ...
    void GetOriginalPriorVariances(const uint8_t* original_prior_data, const int num_priors, vector<vector<uint8_t>> *original_prior_variances) {
        original_prior_variances->clear();

        for (int i = 0; i < num_priors; ++i) {
            int start_idx = (num_priors + i) * 4;
            vector<uint8_t > var;
            for (int j = 0; j < 4; ++j) {
                var.push_back(original_prior_data[start_idx + j]);
            }
            original_prior_variances->push_back(var);
        }
    }
...
// Explicit initialization.
    template void GetDetectionResults(const float *det_data, const int num_det,
                                      const int background_label_id,
                                      map<int, map<int, vector<NormalizedBBox> >
    ...
    void GetTopKScoreIndex(const vector<float> &scores, const vector<int> &indices,
        ...
        TFCHECK_EQ(scores.size(), indices.size());
    ...
    void ApplyNMSFast(const vector<NormalizedBBox> &bboxes,
                      ...
                      vector<int> *indices) {
        // Sanity check.
        TFCHECK_EQ(bboxes.size(), scores.size());
        ...
        while (score_index_vec.size() != 0) {
            ...
            //printf("(%d, %f, %d)\n", idx, score_index_vec.front().first, keep);
            if (keep) {
    ...
    void ComputeAP(const vector<pair<float, int>> &tp, const int num_pos,
        ...
        TFCHECK_EQ(tp.size(), fp.size());
        const int num = tp.size();
        // Make sure that tp and fp have complement value.
        for (int i = 0; i < num; ++i) {
            TFCHECK_LE(fabs(tp[i].first - fp[i].first), eps);
            TFCHECK_EQ(tp[i].second, 1 - fp[i].second);
        }
        prec->clear();
        rec->clear();
        *ap = 0;
        ...
        CumSum(tp, &tp_cumsum);
        TFCHECK_EQ(tp_cumsum.size(), num);

        // Compute cumsum of fp.
        vector<int> fp_cumsum;
        CumSum(fp, &fp_cumsum);
        TFCHECK_EQ(fp_cumsum.size(), num);
        ...
        // Compute recall.
        for (int i = 0; i < num; ++i) {
            TFCHECK_LE(tp_cumsum[i], num_pos);
