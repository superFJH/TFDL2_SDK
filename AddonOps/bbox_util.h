#ifndef TFDL_UTIL_BBOX_UTIL_H_
#define TFDL_UTIL_BBOX_UTIL_H_
...
namespace TFDLBOX {
    ...
    public:
        ...
        void set_xmin(float v) {
            xmin_ = v;
        }

        void set_ymin(float v) {
            ymin_ = v;
        }

        void set_xmax(float v) {
            xmax_ = v;
        }

        void set_ymax(float v) {
            ymax_ = v;
        }

        void set_difficult(float v) {
            difficult_ = v;
        }
...
// Count the number of matches from the match indices.
    int CountNumMatches(const vector<map<int, vector<int>>
...
//      confidence prediction for an image.
    ...
    void GetConfidenceScores(const Dtype *conf_data, const int num,
                             ...
                             vector<map<int, vector<float> > > *conf_scores, int filter = -1);

    void GetConfidenceScores(const uint8_t *conf_data, const int num,
                             const int num_preds_per_class, const int num_classes,
                             vector<map<int, vector<float> > > *conf_scores, int filter = -1);
// Get confidence predictions from conf_data.
//    conf_data: num x num_preds_per_class * num_classes blob.
