...
extern "C" {
    ...
    // External FP16 KV cache ABI.
    //
    // This is intended for heterogeneous prefill/decode deployments where
    // another accelerator owns prefill and llama.cpp owns CPU decode.  Each
    // key/value pointer addresses one layer and uses the contiguous layout:
    //     [n_kv_head, n_tokens, head_dim] (FP16, B = 1)
    //
    // ``positions`` is optional.  When supplied it is stored as
    // [n_pos, n_tokens], matching llama.cpp's internal MRoPE position layout.
    // When omitted, the importer creates a regular one-dimensional sequence
    // beginning at ``first_pos``.  The caller retains ownership of all input
    // buffers for the duration of llama_memory_import_kv_f16().
    struct llama_external_kv_f16 {
        uint32_t n_layer;
        uint32_t n_kv_head;
        uint32_t head_dim;
        uint32_t n_tokens;
    ...
    // Import an externally produced FP16 KV prefix into a standard llama.cpp
    // attention KV cache.  The source is always FP16; compatible target cache
    // types are F16 and Q8_0.  Q8_0 is encoded using ggml's native per-block
    // format during import.  Returns false if the cache is incompatible or
    // the supplied tensor geometry differs from the GGUF model.
    LLAMA_API bool llama_memory_import_kv_f16(
            llama_memory_t mem,
              llama_seq_id seq_id,
