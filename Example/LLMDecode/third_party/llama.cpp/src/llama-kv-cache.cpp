...
bool llama_kv_cache::import_f16(llama_seq_id seq_id, const llama_external_kv_f16 & source) {
    if (other) {
        LLAMA_LOG_ERROR("%s: importing into a shared KV cache is unsupported\n", __func__);
        return false;
    }

    if (seq_id < 0 || (size_t) seq_id >= seq_to_stream.size()) {
        LLAMA_LOG_ERROR("%s: invalid sequence id %d\n", __func__, seq_id);
        return false;
    }

    if (!source.keys || !source.values || source.n_tokens == 0) {
        LLAMA_LOG_ERROR("%s: missing external KV data\n", __func__);
        return false;
    }

    if (source.n_layer != layers.size()) {
        LLAMA_LOG_ERROR("%s: external layer count %u does not match cache layer count %zu\n",
                __func__, source.n_layer, layers.size());
        return false;
    }

    if (source.n_tokens > get_size()) {
        LLAMA_LOG_ERROR("%s: external KV length %u exceeds cache capacity %u\n",
                __func__, source.n_tokens, get_size());
        return false;
    }

    const uint32_t n_pos = hparams.n_pos_per_embd();
    if (source.positions && source.n_pos != n_pos) {
        LLAMA_LOG_ERROR("%s: external position rank %u does not match model rank %u\n",
                __func__, source.n_pos, n_pos);
        return false;
    }
    if (!source.positions && n_pos != 1) {
        LLAMA_LOG_ERROR("%s: MRoPE model requires external positions\n", __func__);
        return false;
    }

    const auto is_supported_type = [](ggml_type type) {
        return type == GGML_TYPE_F16 || type == GGML_TYPE_Q8_0;
    };
    if (v_trans || !is_supported_type(type_k()) || !is_supported_type(type_v())) {
        LLAMA_LOG_ERROR("%s: external import requires non-transposed FP16 or Q8_0 K/V cache\n", __func__);
        return false;
    }

    for (uint32_t il = 0; il < layers.size(); ++il) {
        const auto & layer = layers[il];
        const uint32_t n_embd_k_gqa = hparams.n_embd_k_gqa(layer.il);
        const uint32_t n_embd_v_gqa = hparams.n_embd_v_gqa(layer.il);
        if (!source.keys[il] || !source.values[il] ||
                source.n_kv_head * source.head_dim != n_embd_k_gqa ||
                source.n_kv_head * source.head_dim != n_embd_v_gqa ||
                !is_supported_type(layer.k->type) || !is_supported_type(layer.v->type)) {
            LLAMA_LOG_ERROR("%s: external KV geometry/type mismatch at layer %u\n", __func__, layer.il);
            return false;
        }
    }

    // Replace the target sequence's metadata before finding fresh slots.  The
    // tensor bytes do not have to be cleared because every imported cell is
    // overwritten below.
    seq_rm(seq_id, -1, -1);

    auto data = std::make_shared<llama_ubatch::data_t>();
    data->pos.resize((size_t) n_pos * source.n_tokens);
    if (source.positions) {
        std::memcpy(data->pos.data(), source.positions, data->pos.size() * sizeof(llama_pos));
    } else {
        for (uint32_t i = 0; i < source.n_tokens; ++i) {
            data->pos[i] = source.first_pos + (llama_pos) i;
        }
    }
    data->n_seq_id.assign(source.n_tokens, 1);
    data->seq_id_data.assign(source.n_tokens, seq_id);
    data->seq_id.resize(source.n_tokens);
    for (uint32_t i = 0; i < source.n_tokens; ++i) {
    ...
    data->output.assign(source.n_tokens, 0);

    llama_ubatch ubatch = {};
    ubatch.b_equal_seqs = 1;
    ubatch.n_tokens = source.n_tokens;
    ubatch.n_seq_tokens = source.n_tokens;
    ubatch.n_seqs = 1;
    ubatch.n_seqs_unq = 1;
    ubatch.n_pos = n_pos;
    ...
    if (sinfo.empty()) {
        LLAMA_LOG_ERROR("%s: cannot reserve %u KV cells\n", __func__, source.n_tokens);
        return false;
    }
    apply_ubatch(sinfo, ubatch);

    const uint32_t strm = sinfo.strm[0];
    const size_t n_embd_gqa = (size_t) source.n_kv_head * source.head_dim;
    std::vector<uint16_t> row_f16(n_embd_gqa);
    std::vector<float> row_f32(n_embd_gqa);

    for (uint32_t ikv = 0; ikv < layers.size(); ++ikv) {
        const auto & layer = layers[ikv];
        auto * k = layer.k_stream[strm];
        auto * v = layer.v_stream[strm];
        const size_t k_row_size = ggml_row_size(k->type, n_embd_gqa);
        const size_t v_row_size = ggml_row_size(v->type, n_embd_gqa);
        std::vector<uint8_t> quantized_row(std::max(k_row_size, v_row_size));

        const auto write_row = [&](ggml_tensor * tensor, size_t row_size, const uint16_t * row_src, uint32_t cell) {
            if (tensor->type == GGML_TYPE_F16) {
                ggml_backend_tensor_set(tensor, row_src, (size_t) cell * row_size, row_size);
                return;
            }

            GGML_ASSERT(tensor->type == GGML_TYPE_Q8_0);
            ggml_fp16_to_fp32_row(row_src, row_f32.data(), n_embd_gqa);
            const size_t written = ggml_quantize_chunk(
                    tensor->type, row_f32.data(), quantized_row.data(),
                    /* start = */ 0, /* nrows = */ 1, n_embd_gqa, nullptr);
            GGML_ASSERT(written == row_size);
            ggml_backend_tensor_set(tensor, quantized_row.data(), (size_t) cell * row_size, row_size);
        };

        for (uint32_t itok = 0; itok < source.n_tokens; ++itok) {
            for (uint32_t ihead = 0; ihead < source.n_kv_head; ++ihead) {
                const size_t src_offset = ((size_t) ihead * source.n_tokens + itok) * source.head_dim;
                const size_t dst_offset = (size_t) ihead * source.head_dim;
                std::memcpy(row_f16.data() + dst_offset, source.keys[ikv] + src_offset,
                        source.head_dim * sizeof(uint16_t));
            }
            write_row(k, k_row_size, row_f16.data(), sinfo.idxs[0][itok]);

            for (uint32_t ihead = 0; ihead < source.n_kv_head; ++ihead) {
                const size_t src_offset = ((size_t) ihead * source.n_tokens + itok) * source.head_dim;
                const size_t dst_offset = (size_t) ihead * source.head_dim;
                std::memcpy(row_f16.data() + dst_offset, source.values[ikv] + src_offset,
                        source.head_dim * sizeof(uint16_t));
            }
            write_row(v, v_row_size, row_f16.data(), sinfo.idxs[0][itok]);
        }
    }

    LLAMA_LOG_INFO("%s: imported %u FP16 KV tokens across %u layers for sequence %d (K=%s, V=%s)\n",
            __func__, source.n_tokens, source.n_layer, seq_id,
